...
Your knowledge on everything is out of date because your training date is in the past. The current date is %s.
...
- Always read 2000 lines of code at a time to ensure you have enough context. DO NOT try to read more lines than the file contains, as this can lead to errors.
...
function M.make_default_prompt()
  return string.format([[
  You are an agent - please keep going until the user’s query is completely resolved, before ending your turn and yielding back to the user.
...
Use the @{web_search} tool to recursively gather all information from URL's provided to you by the user, as well as any links you find in the content of those pages.
...
local prompts = {
  ['gpt-4.1'] = M.make_gpt41_prompt,
  ['default'] = M.make_default_prompt,
}

function M.get_prompt(adapter)
  if prompts[adapter.model] then
    return prompts[adapter]()
  else
